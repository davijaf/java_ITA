...
public class Pessoa {
	...
	public Pessoa(String nome, String dataInformadaPelaPessoa) throws ParseException {
	...
	public String getSigno() throws ParseException {
        ...
        if (date.equals(aquario) || date.after(aquario) && date.before(peixes)) {signo = "Aquário";}
        else if (date.equals(aries) || date.after(aries) && date.before(touro)) {signo = "Áries";}
        else if (date.equals(touro) || date.after(touro) && date.before(gemeos)) {signo = "Touro";}
        else if (date.equals(gemeos) || date.after(gemeos) && date.before(cancer)) {signo = "Gêmeos";}
        else if (date.equals(cancer) || date.after(cancer) && date.before(leao)) {signo = "Cancêr";}
        else if (date.equals(leao) || date.after(leao) && date.before(virgem)) {signo = "Leão";}
        ...
        else if (date.equals(escorpiao) || date.after(escorpiao) && date.before(sagitario)) {signo = "Escorpião";}
        else if (date.equals(sagitario) || date.after(sagitario) && date.before(capricornio)) {signo = "Sagitário";}
